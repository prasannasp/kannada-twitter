...
function lists_lists_page($user) {
	...
	$content = "<p><a href='lists/{$user}/memberships'>{$user} ಅವರನ್ನು ಹಿಂಬಾಲಿಸುತ್ತಿರುವ ಲಿಸ್ಟ್'ಗಳು</a> | <strong>{$user} ಅವರು ಹಿಂಬಾಲಿಸುತ್ತಿರುವ ಲಿಸ್ಟ್'ಗಳು</strong></p>";
...
function lists_membership_page($user) {
	...
	$content = "<p><strong>{$user} ಅವರನ್ನು ಹಿಂಬಾಲಿಸುತ್ತಿರುವ ಲಿಸ್ಟ್'ಗಳು</strong> | <a href='lists/{$user}'>{$user}ಅವರು ಹಿಂಬಾಲಿಸುತ್ತಿರುವ ಲಿಸ್ಟ್'ಗಳು</a></p>";
	$content .= theme('lists', $lists);
...
function lists_list_tweets_page($user, $list) {
	...
	$content .= "<p><a href='user/{$user}'>@{$user}</a> ಅವರ ಟ್ವೀಟ್‌ಗಳು/<strong>{$list}</strong> | <a href='{$list_url}/members'>ಸದಸ್ಯರನ್ನು ನೋಡಿ</a> | <a href='{$list_url}/subscribers'>ಚಂದಾದಾರರನ್ನು ವೀಕ್ಷಿಸಿ</a></p>";
...
function theme_list_pagination($json) {
	if ($cursor = (string) $json->next_cursor) {
		$links[] = "<a href='{$_GET['q']}?cursor={$cursor}'>ಮುಂದಿನ</a>";
	}
	if ($cursor = (string) $json->previous_cursor) {
		$links[] = "<a href='{$_GET['q']}?cursor={$cursor}'>ಹಿಂದಿನ</a>";
	}
